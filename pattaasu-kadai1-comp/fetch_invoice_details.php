...
try {
    ...
    // Fetch all products where is_delete = 0
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $prod_code = $ref_product['productId'];
        $product = $latest_by_code[$prod_code] ?? null;
        if (!$product) {
            continue;
        }
        $items[] = [
            'productId' => $referenced_id,
            'name' => $product['name'],
            'tamilName' => $product['tamilName'],
            'quantity' => (int)$row['quantity'],
            'price' => (float)$product['offerPrice'], // Latest offer_price
            'purchasedPrice' => (float)$row['price'], // Price from invoice_items
            'amount' => (float)$row['quantity'] * (float)$product['offerPrice']
        ];
    }
    $stmt->close();
